...
def graphviz_command_draw(file_format='svg'):
    return "neato -Gforcelabels=false -Nfontname=\"%s\" -Efontname=\"%s\" -Ecolor=grey  -Gsize=60,60! -n2 -T%s" % (CHINESE_FONT, CHINESE_FONT, file_format)

def graphviz_command_scale(s1, s2):
    script = CURPATH + "/external/utils/change_size.gvpr"

    return "gvpr -c -a %s -f %s | neato -Nfontname=\"%s\" -Efontname=\"%s\" -Gsize=%s! -Ecolor=grey -n2 -Tsvg" % (s1, script, CHINESE_FONT, CHINESE_FONT, s2)
...
def call_process(gv_command, map_string, raw_output = False):
    ...
    input_data = map_string
    if isinstance(map_string, unicode):
        input_data = map_string.encode('utf-8')
    else:

        input_data = str(map_string)

    proc = Popen(gv_command, stdout=PIPE, stdin=PIPE, stderr=PIPE, shell=True)

    dot_out, map_err = proc.communicate(input = input_data)
